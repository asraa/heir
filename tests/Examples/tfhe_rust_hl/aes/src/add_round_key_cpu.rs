use tfhe::{ClientKey, ConfigBuilder, generate_keys, set_server_key, FheUint8};
use tfhe::prelude::*;
use std::time::Instant;

mod add_round_key;

pub fn encrypt_block(value: [u8; 16], client_key: &ClientKey) -> Vec<FheUint8> {
    value.into_iter().map(|v| FheUint8::encrypt(v, client_key)).collect()
}

pub fn decrypt_block(ciphertexts: &Vec<FheUint8>, client_key: &ClientKey) -> Vec<u8> {
...
fn main() {
    ...
    let result = add_round_key::add_round_key(&server_key, &encrypted_block, &encrypted_key);
