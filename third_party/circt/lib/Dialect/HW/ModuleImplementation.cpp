...
static ParseResult parseFunctionResultList(
    OpAsmParser &parser, SmallVectorImpl<Attribute> &resultNames,
  ...
  auto parseElt = [&]() -> ParseResult {
    ...
    if (parser.parseColonType(resultTypes.back())) return failure();

    // Parse the result attributes.
    NamedAttrList attrs;
    if (failed(parser.parseOptionalAttrDict(attrs))) return failure();
...
static StringRef getModuleArgumentName(Operation *module, size_t argNo) {
  if (auto mod = dyn_cast<HWModuleLike>(module)) {
    if (argNo < mod.getNumInputPorts()) return mod.getInputName(argNo);
    return StringRef();
  }
...
static StringRef getModuleResultName(Operation *module, size_t resultNo) {
  if (auto mod = dyn_cast<HWModuleLike>(module)) {
    if (resultNo < mod.getNumOutputPorts()) return mod.getOutputName(resultNo);
    return StringRef();
  }
...
void module_like_impl::printModuleSignature(OpAsmPrinter &p, Operation *op,
  ...
  if (funcOp) {
    if (auto args = funcOp.getAllArgAttrs())
      for (auto a : args.getValue()) inputAttrs.push_back(a);
    inputAttrs.resize(funcOp.getNumArguments());
    if (auto results = funcOp.getAllResultAttrs())
      for (auto a : results.getValue()) outputAttrs.push_back(a);
  ...
  for (unsigned i = 0, e = argTypes.size(); i < e; ++i) {
    ...
    if (!isExternal) {
      ...
      if (tmpStream.str().drop_front() != argName) needArgNamesAttr = true;
    ...
    if (flags.shouldPrintDebugInfo()) {
      ...
      if (!isa<UnknownLoc>(loc)) p.printOptionalLocationSpecifier(loc);
    }
  }
...
static ParseResult parsePortList(
    OpAsmParser &p, SmallVectorImpl<module_like_impl::PortParse> &result) {
...
ParseResult module_like_impl::parseModuleSignature(
  ...
  // Parse the port list.
  if (parsePortList(parser, args)) return failure();
...
static const char *directionAsString(ModulePort::Direction dir) {
  if (dir == ModulePort::Direction::Input) return "input";
  if (dir == ModulePort::Direction::Output) return "output";
  if (dir == ModulePort::Direction::InOut) return "inout";
